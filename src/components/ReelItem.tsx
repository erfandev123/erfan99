...
export const ReelItem: React.FC<ReelItemProps> = React.memo(({ reel, isModal, onClose }) => {
  ...
  const { currentUser, setViewingUser, setCurrentPage, setViewingReel, highlightedCommentId, highlightedPostId, setHighlightedPostId, setShowLikesList, setTargetLikesPostId, setShowViewsList, setTargetViewsPostId, setShowAnalytics, setTargetAnalyticsPostId } = useAppStore();
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      import('firebase/firestore').then(m => {
        ...
        const followRef = m.doc(db, 'users', currentUser.uid, 'following', reel.authorId);
    ...
    return () => unsub();
  }, [reel.id, currentUser, reel.authorId]);
  ...
  return (
    <div ref={containerRef} className="w-full h-full snap-start snap-always flex items-center justify-center relative bg-white sm:bg-[#f8f9fa]">
      {isNearScreen && (
        ...
        <div 
          onClick={handleTap}
          className="relative w-full h-full md:h-[min(90dvh,850px)] md:w-auto md:min-w-[350px] md:aspect-[9/16] bg-black sm:rounded-[3px] overflow-hidden md:shadow-[0_0_15px_rgba(0,0,0,0.05)] cursor-pointer flex items-center justify-center group/video shrink-0"
        >
          <video 
            ref={videoRef}
            src={reel.media?.[0]} 
          ...
          <div className="md:hidden absolute bottom-3 left-4 right-4 z-10 text-left flex flex-col pointer-events-auto">
            {isCurrentUser && (
              <button 
                ...
                className="mb-2 bg-black/40 hover:bg-black/60 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-lg border border-white/20 transition-all active:scale-95 flex items-center space-x-1.5 self-start"
              >
            ...
            <div className="flex items-center space-x-2 text-white/80 overflow-hidden max-w-[200px]">
          ...
          <div className="md:hidden absolute right-4 bottom-[calc(max(20px,env(safe-area-inset-bottom)+20px))] flex flex-col items-center space-y-5 z-20 pointer-events-auto">
            <button 
              onClick={(e) => { e.stopPropagation(); handleLike(); }} 
              className="flex flex-col items-center space-y-1 group transition-transform active:scale-90"
            >
              <Heart className={`w-[26px] h-[26px] drop-shadow-md transition-transform active:scale-125 ${liked ? 'fill-red-500 text-red-500' : 'text-white'}`} strokeWidth={2.5} />
              <span className="text-[12px] font-bold text-white drop-shadow-md leading-tight">{likesCount > 0 ? likesCount : 'Like'}</span>
            </button>
            ...
            <button onClick={(e) => { e.stopPropagation(); setShowComments(true); }} className="flex flex-col items-center space-y-1 group transition-transform active:scale-90">
              <MessageCircle className="w-[26px] h-[26px] text-white drop-shadow-md" strokeWidth={2.5} />
              <span className="text-[12px] font-bold text-white drop-shadow-md leading-tight">{commentsCount > 0 ? commentsCount : 'Comment'}</span>
            </button>
            ...
            <button onClick={(e) => { e.stopPropagation(); setShowShare(true); }} className="flex flex-col items-center space-y-1 group transition-transform active:scale-90">
              <Send className="w-[26px] h-[26px] text-white drop-shadow-md" strokeWidth={2.5} />
              <span className="text-[12px] font-bold text-white drop-shadow-md leading-tight">Share</span>
            </button>
            ...
            <button onClick={(e) => { e.stopPropagation(); handleFavorite(); }} className="flex flex-col items-center space-y-1 group transition-transform active:scale-90">
              <Bookmark className={`w-[26px] h-[26px] drop-shadow-md ${saved ? 'fill-white text-white' : 'text-white'}`} strokeWidth={2.5} />
              <span className="text-[12px] font-bold text-white drop-shadow-md leading-tight">Save</span>
            </button>
            
            <button onClick={(e) => { e.stopPropagation(); setShowMoreActions(!showMoreActions); }} className="flex flex-col items-center group transition-transform active:scale-90">
              <MoreHorizontal className="w-[26px] h-[26px] text-white drop-shadow-md mt-2" strokeWidth={2.5} />
            </button>
          </div>
          ...
          {/* Progress Bar */}
          <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-white/20 z-30 md:hidden">
            <div className="h-full bg-white" style={{ width: `${progress}%` }}></div>
          </div>
        </div>

        {/* Desktop Right Actions Column */}
        <div className="hidden md:flex flex-col items-center justify-end space-y-5 pb-8 pl-4 w-[60px] shrink-0">
          <button 
            onClick={(e) => { e.stopPropagation(); handleLike(); }} 
            className="flex flex-col items-center space-y-1.5 group transition-transform hover:scale-105 active:scale-95"
          >
            <div className="p-3 bg-white hover:bg-gray-50 rounded-full shadow-sm border border-gray-100">
              <Heart className={`w-6 h-6 transition-transform ${liked ? 'fill-red-500 text-red-500' : 'text-gray-900'}`} strokeWidth={2} />
            </div>
            <span className="text-[13px] font-semibold text-gray-700">{likesCount > 0 ? (likesCount > 1000 ? (likesCount/1000).toFixed(1)+'K' : likesCount) : 'Like'}</span>
          </button>
          ...
          >
            ...
            <span className="text-[13px] font-semibold text-gray-700">{commentsCount > 0 ? commentsCount : 'Comment'}</span>
          </button>
          ...
          >
            <div className="p-3 bg-white hover:bg-gray-50 rounded-full shadow-sm border border-gray-100">
              <Send className="w-6 h-6 text-gray-900" strokeWidth={2} />
            </div>
          </button>
          ...
          >
            <div className="p-3 bg-white hover:bg-gray-50 rounded-full shadow-sm border border-gray-100">
              <Bookmark className={`w-6 h-6 ${saved ? 'fill-black text-black' : 'text-gray-900'}`} strokeWidth={2} />
            </div>
          </button>
          ...
          >
            <div className="p-3 bg-white hover:bg-gray-50 rounded-full shadow-sm border border-gray-100">
              <MoreHorizontal className="w-6 h-6 text-gray-900" strokeWidth={2} />
            </div>
          </button>
...
const CommentItem = React.memo(({ comment, postId, authorId, authorName, authorAvatar, media, onReply }: { comment: Comment, postId: string, authorId: string, authorName: string, authorAvatar: string, media?: string, onReply: (id: string, name: string, authorId: string) => void }) => {
  ...
  const { currentUser, setViewingUser, setCurrentPage, highlightedCommentId, setHighlightedCommentId } = useAppStore();
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-gray-100 flex flex-col space-y-2 bg-white sticky bottom-0">
          ...
          <div className="flex items-center space-x-3">
            <img src={currentUser?.avatar} className="w-9 h-9 rounded-full object-cover hidden sm:block shadow-sm" referrerPolicy="no-referrer" />
            <div className="flex-1 relative">
              <input 
                ref={inputRef}
                value={text} 
                onChange={handleInputChange}
                placeholder={replyingTo ? `Reply to ${replyingTo.name}...` : `Comment as ${currentUser?.name.split(' ')[0]}...`} 
                className="w-full bg-gray-100 hover:bg-gray-200 focus:bg-white focus:ring-1 focus:ring-blue-100 transition-all rounded-full px-5 py-3 text-sm outline-none border border-transparent focus:border-blue-400" 
              />
            </div>
            <button disabled={!text.trim()} className="text-blue-500 hover:text-blue-600 font-bold disabled:opacity-30 disabled:cursor-not-allowed transition-all px-2">Post</button>
          </div>
        </form>
      </div>
    </div>, document.body
...
export const SharePortal = ({ reel, onClose }: { reel: Post, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-end md:items-center justify-center bg-black/80 md:p-4" onClick={(e) => e.stopPropagation()}>
