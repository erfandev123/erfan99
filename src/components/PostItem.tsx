...
import { toggleLike, addComment, getComments, deletePost, deleteComment, toggleCommentLike, toggleFavorite } from '../services/postService';
...
export const PostItem: React.FC<PostItemProps> = React.memo(({ post }) => {
  const { setViewingUser, setViewingMedia, currentUser, setCurrentPage, setActiveChat, setShowAnalytics, setTargetAnalyticsPostId } = useAppStore();
  ...
  return (
    <div 
      className="bg-white md:rounded-2xl border-y md:border border-gray-100 shadow-sm overflow-hidden mb-2 md:mb-3 relative hover:shadow-md transition-shadow duration-300"
        ...
        <div className="w-full bg-black overflow-hidden border-y border-gray-100 flex justify-center">
          {post.media.length === 1 ? (
            <div className="w-full max-h-[600px] overflow-hidden flex items-center justify-center bg-black">
              {post.type === 'reel' || post.media[0].includes('.mp4') || post.media[0].includes('video') ? (
                <video 
                  src={post.media[0]} 
                  controls 
                ...
                <img 
                  ...
                  loading="lazy" 
                  referrerPolicy="no-referrer"
                  className="w-full h-auto max-h-[600px] object-cover sm:object-contain cursor-pointer transition-all duration-300" 
      ...
      <div className="p-3 relative">
        <div className="flex items-center justify-between mb-3">
          ...
          >
            ...
            <span className="text-[13px] text-gray-500 font-bold">{likesCount.toLocaleString()} likes</span>
          </div>
          <div className="flex text-[13px] text-gray-500 font-bold space-x-3">
            {post.viewsCount !== undefined && currentUser?.uid === post.authorId && (
              <span className="hover:underline cursor-pointer" onClick={() => { setTargetViewsPostId(post.id); setShowViewsList(true); }}>{post.viewsCount.toLocaleString()} views</span>
            )}
            <span className="hover:underline cursor-pointer" onClick={() => setShowCommentsModal(true)}>{post.commentsCount} comments</span>
          </div>
        </div>
