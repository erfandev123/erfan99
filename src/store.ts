...
export const useAppStore = create<AppState>((set) => ({
  currentPage: (localStorage.getItem('lastVisitedPage') as PageType) || 'home',
  setCurrentPage: (page) => {
  ...
  popPage: () => set((state) => {
    ...
    localStorage.setItem('lastVisitedPage', prevPage);
    return { 
  ...
  feedScrollY: Number(sessionStorage.getItem('feedScrollY')) || 0,
  setFeedScrollY: (y) => {
    sessionStorage.setItem('feedScrollY', y.toString());
    set({ feedScrollY: y });
  },
  reelsScrollY: Number(sessionStorage.getItem('reelsScrollY')) || 0,
  setReelsScrollY: (y) => {
    sessionStorage.setItem('reelsScrollY', y.toString());
    set({ reelsScrollY: y });
  },
}));
