...
const InAppToaster = memo(() => {
  const { currentUser, setViewingMedia, setViewingReel, setCurrentPage, setViewingUser, setHighlightedCommentId, setHighlightedPostId, setActiveChat, activeChat } = useAppStore();
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div 
          className="bg-white shadow-xl border border-gray-100 rounded-2xl flex items-center p-3 pointer-events-auto cursor-pointer max-w-sm w-full transition-transform active:scale-95"
          onClick={async () => {
             setToast(null);
...
export default function App() {
  ...
  useEffect(() => {
    // Page restoration is handled directly by store initialization now
  }, []);
  ...
  return (
    <div className="flex h-screen w-full bg-white text-black overflow-hidden font-sans relative">
      {/* Super Fast Background Loading Splash Screen Overlay */}
      <AnimatePresence>
        {(showSplash || isAuthLoading) && (
          ...
          >
            ...
            >
              <div className="w-20 h-20 bg-gradient-to-br from-blue-500 to-purple-500 rounded-3xl flex items-center justify-center text-white font-extrabold text-4xl shadow-[0_8px_30px_rgb(59,130,246,0.3)] mb-4">
                R
              </div>
      ...
      {viewingReel && (
        <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center animate-in fade-in duration-200">
          <div id="global-reels-container" className="h-[100dvh] w-full overflow-y-auto snap-y snap-mandatory hide-scrollbar overscroll-none overflow-x-hidden">
...
function ViewsList() {
  const { showViewsList, setShowViewsList, targetViewsPostId, setTargetViewsPostId, setViewingUser, setCurrentPage } = useAppStore();
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {loading ? (
                ...
                viewers.map((user) => (
                  <div key={user.uid} className="flex items-center space-x-3 p-3 hover:bg-gray-50 rounded-2xl cursor-pointer" onClick={() => { setViewingUser(user); setCurrentPage('profile'); setShowViewsList(false); }}>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black animate-in fade-in duration-300">
      ...
      <div className="w-full h-full sm:h-[90vh] sm:max-w-[450px] bg-black sm:rounded-[2.5rem] overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 bg-gradient-to-t from-black/80 to-transparent flex items-center space-x-2 sm:space-x-4 z-30 pb-safe">
          ...
          <button className="p-2 sm:p-3 text-white hover:text-red-500 transition-colors active:scale-110 shrink-0"><Heart className="w-6 h-6 sm:w-7 sm:h-7" /></button>
          <button className="p-2 sm:p-3 text-white hover:text-blue-400 transition-colors active:scale-110 shrink-0"><Send className="w-6 h-6 sm:w-7 sm:h-7" /></button>
        </div>
      </div>
    </div>
  );
