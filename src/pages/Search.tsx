...
export default function Search() {
  ...
  const { currentUser, setViewingUser, setCurrentPage, setViewingMedia, setViewingReel, setViewingReelContext } = useAppStore();
  ...
  return (
    <div className="h-full w-full overflow-y-auto bg-white flex flex-col items-center no-scrollbar">
      ...
      <div className="w-full max-w-[600px] pb-24">
          ...
          <div className="flex flex-col py-2">
            {searchResults.map(user => (
              <div key={user.uid} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer group">
                <div className="flex items-center space-x-3 flex-1 min-w-0" onClick={() => { setViewingUser(user); setCurrentPage('profile'); }}>
        ...
        {activeTab === 'videos' && (
          <div className="grid grid-cols-3 gap-[2px] sm:gap-1 mt-[1px]">
            {currentPosts.map(post => (
              <div 
                key={post.id} 
                className="relative aspect-[9/16] bg-gray-100 group cursor-pointer overflow-hidden sm:rounded-md shadow-sm"
                onClick={() => {
                  if (post.type === 'reel') {
                    setViewingReel(post);
                    setViewingReelContext(searchQuery ? 'search' : 'feed');
                  } else {
                  ...
                  <>
                    {post.type === 'reel' || post.media[0].includes('.mp4') || post.media[0].includes('video') ? (
                      <div className="w-full h-full relative">
                        <video src={post.media[0]} className="w-full h-full object-cover" preload="metadata" muted />
                        <div className="absolute top-2 right-2 p-1 bg-black/20 backdrop-blur-md rounded-md flex items-center justify-center">
                          <Play className="w-3 h-3 text-white fill-white" />
                        </div>
                      </div>
                    ) : (
                      <img src={post.media[0]} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" loading="lazy" />
                    )}
                  </>
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-blue-500 to-purple-600 p-4 flex items-center justify-center">
                    <p className="text-[10px] text-white font-black text-center line-clamp-4">{post.text}</p>
                  </div>
                )}
