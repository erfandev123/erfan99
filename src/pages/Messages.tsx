...
import { Search, Video, Phone, MoreVertical, Image as ImageIcon, Mic, Smile, Plus, ArrowLeft, Check, CheckCheck, Info, ChevronDown, Send, X, Heart, Type, Music, MoreHorizontal, UserPlus, Bell, MessageCircle, Play, Reply, Trash2 } from 'lucide-react';
...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeMessages(activeChat, setMessages);
    const unsubscribeTyping = subscribeTyping(activeChat, setTypingUsers);
    return () => {
      unsubscribe();
      unsubscribeTyping();
    };
  ...
  return (
      ...
      <div className={`w-full md:w-[350px] flex-shrink-0 border-r border-gray-100 flex flex-col h-full bg-white transition-transform duration-300 ${activeChat ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="p-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div className="flex items-center space-x-2 cursor-pointer">
            <h1 className="text-xl font-black tracking-tighter bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 bg-clip-text text-transparent italic drop-shadow-sm">Reeta</h1>
    ...
    {/* Right Chat Area */}
      {activeChat ? (
        isSpecialChat ? (
          <div className="fixed inset-0 z-[100] md:relative md:z-auto flex-1 flex flex-col h-full bg-white transition-colors duration-300">
        ...
        <div className={`fixed inset-0 z-[100] md:relative md:z-auto flex-1 flex flex-col h-full ${chatTheme} relative ${!activeChat ? 'hidden md:flex' : 'flex'} transition-colors duration-300`}>
          ...
          <div className={`h-[72px] px-4 border-b border-gray-100 flex items-center justify-between ${chatTheme === 'bg-white' ? 'bg-white/90' : 'bg-transparent'} backdrop-blur-md sticky top-0 z-20 transition-colors duration-300`}>
            <div className="flex items-center space-x-3">
              ...
              <img src={activeContact?.avatar} alt="Avatar" className="w-11 h-11 rounded-full object-cover shadow-sm border border-gray-100" />
              <div>
            ...
            <div className="flex items-center space-x-2">
              <button onClick={() => setShowThemeSettings(true)} className="p-2.5 hover:bg-black/5 rounded-full transition-colors"><Info className={`w-6 h-6 ${chatTheme === 'bg-white' ? 'text-gray-900' : 'text-gray-800'}`} strokeWidth={1.5} /></button>
            </div>
          </div>
                ...
                >
                  ...
                  <div className={`flex flex-col max-w-[75%] ${isMe ? 'items-end' : 'items-start'}`}>
                    <div 
                      ...
                      className={`relative flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-full`}
                    >
                      {/* Reply preview inside message bubble if it's a reply */}
                      {msg.replyTo && (
                         <div 
                           className={`mb-1 px-2 py-1 rounded-md text-[11px] opacity-80 border-l-[3px] border-blue-400 cursor-pointer hover:opacity-100 transition-opacity max-w-full flex items-center ${isMe ? 'bg-black/10 text-white border-white/40' : 'bg-black/5 text-gray-800'}`}
                           onClick={() => {
                         ...
                         >
                            ...
                            {msg.replyTo.type === 'reel' && msg.replyTo.mediaUrl && (
                               <div className="w-6 h-8 rounded shrink-0 overflow-hidden ml-1">
                                 <video src={`${msg.replyTo.mediaUrl}#t=0.001`} className="w-full h-full object-cover" preload="metadata" />
                               </div>
                            )}
                         </div>
                      )}
                      <div className="max-w-full relative">
                        {(() => {
                          ...
                          const isEmojiOnly = emojiRegex.test(msg.content.trim()) && Array.from(msg.content.trim()).length <= 5;
                          ...
                          if (msg.type === 'reel') {
                            return (
                              <div className="relative w-48 aspect-[9/16] rounded-2xl overflow-hidden cursor-pointer shadow-md bg-black group"
                                  ...
                                  <video src={`${msg.mediaUrl}#t=0.001`} className="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform" preload="metadata" />
                            ...
                            return (
                              <div className="rounded-2xl overflow-hidden shadow-sm max-w-[240px] cursor-pointer border border-gray-100" onClick={() => setViewingMedia({ type: 'image', url: msg.mediaUrl!, user: { name: activeContact?.name || '', avatar: activeContact?.avatar || '' } })}>
                                <img src={msg.mediaUrl} alt="Sent image" className="w-full h-auto object-cover" />
                              </div>
                            );
                          } else if (isEmojiOnly) {
                            return <div className="text-[44px] leading-none drop-shadow-sm px-2 py-1">{msg.content}</div>;
                          } else {
                            return (
          ...
          {/* Input Area */}
          <div className={`p-3 md:p-4 ${chatTheme === 'bg-white' ? 'bg-white' : 'bg-transparent'} z-10 pb-6 transition-colors duration-300 w-full flex flex-col`}>
            {replyingTo && (
              <div className="flex items-center justify-between bg-gray-100/90 rounded-lg px-2 py-1.5 border border-gray-200 mx-1 mb-1 z-0 shadow-sm relative">
                <div className="flex-1 min-w-0 border-l-[3px] border-blue-500 pl-2 flex items-center">
                  ...
                  <p className="text-[12px] font-bold text-gray-700 truncate leading-tight">{replyingTo.content || replyingTo.type}</p>
                </div>
                <button onClick={() => setReplyingTo(null)} className="p-1 hover:bg-gray-200 rounded-full transition-colors ml-2 shrink-0">
            ...
            <form onSubmit={handleSend} className="flex items-center space-x-2 md:space-x-3 max-w-full relative z-10">
              <div className={`flex-1 ${chatTheme === 'bg-white' ? 'bg-gray-50 hover:bg-gray-100' : 'bg-white/80 hover:bg-white'} rounded-[24px] flex items-center px-3 md:px-4 py-1.5 md:py-2 border border-gray-200 focus-within:border-blue-200 focus-within:bg-white transition-all shadow-sm overflow-hidden`}>
                <button type="button" className="p-1.5 hover:bg-black/5 rounded-full mr-1 md:mr-2 transition-colors shrink-0"><Smile className="w-5 h-5 md:w-6 md:h-6 text-gray-500" strokeWidth={1.5} /></button>
                <input 
                  type="text" 
                  value={inputText}
                  ...
                  <div className="flex items-center space-x-0.5 md:space-x-1 ml-1 md:ml-2 shrink-0">
                    {isRecording ? (
                      <div className="flex items-center space-x-2 mr-2">
                        ...
                        <span className="text-red-500 text-sm font-medium w-10">{Math.floor(recordingDuration / 60)}:{(recordingDuration % 60).toString().padStart(2, '0')}</span>
                        <button type="button" onClick={stopRecording} className="p-1 px-3 bg-red-50 hover:bg-red-100 text-red-500 rounded-full font-bold text-sm transition-colors">Send</button>
                      </div>
                    ) : (
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (storyImage) {
                    ...
                    fileToUpload = new File([blob], 'story.jpg', { type: blob.type });
                  }
