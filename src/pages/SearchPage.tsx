...
export default function SearchPage() {
  ...
  const { setViewingMedia, setViewingUser, setViewingReel, setCurrentPage, currentUser } = useAppStore();
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        const postResults = postsSnap.docs.map(doc => ({ id: doc.id, ...doc.data(), searchType: 'post' }));
  ...
  return (
    <div className="h-full w-full overflow-y-auto bg-[#fafafa] flex flex-col items-center pb-20">
      ...
      <div className="sm:hidden w-full px-4 py-4 border-b border-gray-100 flex items-center space-x-4 bg-white sticky top-0 z-20">
        <button onClick={() => setCurrentPage('home')} className="p-2 -ml-2 hover:bg-gray-100 rounded-full transition-colors">
                ...
                >
                  {item.media?.[0] ? (
                    <>
                      {item.type === 'reel' ? (
                        <div className="w-full h-full relative">
                          <video src={item.media[0]} className="w-full h-full object-cover" muted />
                          <div className="absolute inset-0 bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <Play className="w-8 h-8 text-white fill-white" />
                          </div>
